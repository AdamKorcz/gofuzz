...
func (c Continue) FuzzNoCustom(obj interface{}) error {
	...
	v = v.Elem()
	fmt.Println("Should now print 'We are calling FuzzNoCustom with f.isGoFuzz'")
	if c.fc.fuzzer.isGoFuzz {
		fmt.Println("We are calling FuzzNoCustom with f.isGoFuzz")
		err := c.fc.doGoFuzz(v, flagNoCustomFuzz)
		if err != nil {
			return err
		}
	}else{
...
func (c Continue) GetGoFuzzInt() (int, error) {
...
func (f *Fuzzer) GetGoFuzzUint() (uint64, error) {
	if f.position >= len(f.data) {
		return 0, errors.New("Not enough bytes to create int")
	}
...
func (c Continue) GetGoFuzzUint() (uint64, error) {
	if c.fc.fuzzer.position >= len(c.fc.fuzzer.data) {
		return 0, errors.New("Not enough bytes to create int")
	}
...
func (c Continue) GetGoFuzzBool() (bool, error) {
	if c.fc.fuzzer.position >= len(c.fc.fuzzer.data) {
		return true, errors.New("Not enough bytes to create int")
	}
	n := int(c.fc.fuzzer.data[c.fc.fuzzer.position])
	c.fc.fuzzer.position++
	if (n % 2) == 0 {
		return true, nil
	} else {
